 ...
 */
export async function getMarketPriceHistory(tokenId: string, interval: "1h" | "1d" = "1h") {
  ...
  if (history.length > 0) {
    ...
    console.log(`- Change: ${change.direction === "up" ? "+" : "-"}${change.percentage.toFixed(2)}%`);
  }
...
export async function analyzeTrading(marketId: string) {
  ...
  const totalBuyVolume = buyTrades.reduce((sum, t) => sum + parseFloat(t.size), 0);
  const totalSellVolume = sellTrades.reduce((sum, t) => sum + parseFloat(t.size), 0);
  
  console.log(`Trading Analysis for Market ${marketId}:`);
  console.log(`- Total Trades: ${trades.length}`);
  console.log(`- Buy Trades: ${buyTrades.length} (${formatNumber(totalBuyVolume)} volume)`);
  console.log(`- Sell Trades: ${sellTrades.length} (${formatNumber(totalSellVolume)} volume)`);
...
export async function getMarketWhales(marketId: string) {
  ...
  topHolders.forEach((holder, idx) => {
    console.log(`${idx + 1}. ${holder.address.slice(0, 10)}... - ${formatNumber(holder.size)} (${holder.outcome})`);
  });
...
export async function monitorEventStats(eventId: string) {
  ...
  if (event.markets && event.markets.length > 0) {
    ...
    try {
      ...
      console.log(`- Open Interest: ${formatNumber(openInterest.open_interest)}`);
    } catch (error) {
...
export async function completeMarketAnalysis(searchQuery: string) {
  ...
  if (fullEvent.markets && fullEvent.markets.length > 0) {
    ...
    if (tokenId) {
      ...
      if (history.length > 1) {
        const change = calculateChange(history[0].p, history[history.length - 1].p);
 ...
 */
export function displayMarketSummary(market: any, prices: any[]) {
  console.log("\n" + "=".repeat(50));
