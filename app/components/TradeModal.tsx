...
export default function TradeModal({
  ...
  const { currentTokenId, opposingTokenId } = useMemo(() => {
    if (!market.clobTokenIds) return { currentTokenId: null, opposingTokenId: null };
    ...
    const currentIndex = outcomes.findIndex((o: string) => o.toLowerCase() === outcome.toLowerCase());
    
    if (currentIndex === -1) {
        // Fallback for binary simple case
        if (outcome.toLowerCase() === "yes") return { currentTokenId: ids[0], opposingTokenId: ids[1] };
        if (outcome.toLowerCase() === "no") return { currentTokenId: ids[1], opposingTokenId: ids[0] };
  ...
  }, [isOpen, market, outcome, currentTokenId, opposingTokenId]);
  ...
  const fetchLiquidity = async () => {
    ...
    try {
      ...
      const currentPromise = fetch(`/api/orderbook?tokenId=${currentTokenId}`).then(res => res.json());
      
      // Fetch opposing token order book if exists
      const opposingPromise = opposingTokenId 
        ? fetch(`/api/orderbook?tokenId=${opposingTokenId}`).then(res => res.json())
        : Promise.resolve(null);

      const [currentData, opposingData] = await Promise.all([currentPromise, opposingPromise]);
        ...
        <div className="p-6">
            {loading ? (
                ...
                <div className="space-y-6">
                    ...
                    {/* Buy Button */}
                    <button className={`w-full py-3.5 rounded-lg font-bold text-lg shadow-lg transition-all transform active:scale-[0.98] ${
                        outcome === 'Yes' 
                    ...
                    <div className="text-center">
                        <p className="text-gray-400 text-sm">
                            Buy {outcome} up to <span className="text-white font-mono font-medium">${maxBuyAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </p>
                    </div>
                </div>
            )}
